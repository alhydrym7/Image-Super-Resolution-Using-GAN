...
def train(epochs, batch_size, input_dir, output_dir, model_save_dir, number_of_images, train_test_ratio):
    ...
    for e in range(1, epochs+1):
        ...
        if e == 1 or e % 5 == 0:
            Utils.plot_generated_images(output_dir, e, generator, x_test_hr, x_test_lr)
        if e % 500 == 0:
            generator.save(model_save_dir + 'gan_model%d.h5' % e)
            discriminator.save(model_save_dir + 'dis_model%d.h5' % e)
...
if __name__== "__main__":
    ...
    parser.add_argument('-i', '--input_dir', action='store', dest='input_dir', default='./dataset/' ,
                    help='Path for input images')
    ...
    parser.add_argument('-n', '--number_of_images', action='store', dest='number_of_images', default=700 ,
                    help='Number of Images', type= int)
